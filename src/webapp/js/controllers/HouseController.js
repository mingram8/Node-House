...
app.controller('HouseController', function ($scope, $http, $sce, $interval, $timeout) {
    ...
    $scope.getWeather = function () {
        $http.get('/getWeather').success(function (data) {
            $scope.weather = data.weather.weather.item.condition;
            $scope.astronomy = {};
            var hours = data.weather.weather.astronomy.sunrise.split(":");
            var minutes = hours[1].split(" ");
            hours = hours[0];
            $scope.astronomy.sunrise = {hours: parseInt(hours), minutes: parseInt(minutes)}
            var hours = data.weather.weather.astronomy.sunset.split(":");
            var minutes = hours[1].split(" ");
            hours = hours[0];
            $scope.astronomy.sunset = {hours: parseInt(hours) + 12, minutes: parseInt(minutes)}
            try {
